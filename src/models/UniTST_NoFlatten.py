...
from src.layers.embed import PatchEmbedding, PatchReconstruction, MLPEmbedding
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if 'conditional' in configs.task_name:
            ...
            if configs.dispatchers > 0:
                ...
                if configs.RoPE:
                    attention_layer = AttentionLayer_w_dispatchers(attention, configs.d_model, configs.n_heads,
                                                                   configs.dispatchers,
        ...
        # Prediction Head
        self.head = PredictionHead(configs.d_model, configs.patch_len, head_dropout=configs.dropout)
        self.reconstruct_head = PatchReconstruction(self.seq_len, configs.patch_len, configs.stride)
    ...
    def basic_diffusion_forward(self, x_enc, t):
        ...
        # Decoder
        dec_out = self.head(enc_out)  # z: [bs x nvars x patch_num x patch_len]
        embed = nn.Flatten(start_dim=-2)(enc_out)
        dec_out = self.reconstruct_head(dec_out)  # z: [bs x nvars x target_window]
    ...
    def create_batch_causal_mask(self, x):
        B, L, _ = x.shape
        num_assets = L // self.num_patches
        single_mask = torch.ones((L, L), dtype=torch.bool)
        start_idxs = [i * self.num_patches for i in range(num_assets)]

        for i in range(L):
            idx = i % self.num_patches
            end_idxs = [start_idxs[j] + idx + 1 for j in range(num_assets)]
            for j in range(len(start_idxs)):
                single_mask[i, start_idxs[j]:end_idxs[j]] = False

        return single_mask.unsqueeze(0).expand(B, -1, -1).to(x.device)