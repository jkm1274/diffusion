    ...
    def basic_diffusion_forward(self, x_enc, t):
        ...
        for i in range(len(self.patch_len)):
            ...
            if self.channel_embedding is not None:
                ...
                channel_embed = None
            # print(f"[Patch {i}] enc_out shape: {enc_out.shape}")  # (B, L, D)
            # print(f"[Patch {i}] expected attn_mask shape: {self.attn_mask_list[i].shape}")
            # print(f"[Patch {i}] num_patches: {self.num_patches[i]}, n_vars: {n_vars}")
            # print(f"[Patch {i}] L (expected): {self.num_patches[i] * self.num_assets}")
            # print(f"[Patch {i}] actual L: {enc_out.shape[1]}")
            attn_mask = torch.triu(torch.ones(enc_out.shape[1], enc_out.shape[1], dtype=torch.bool), diagonal=1).to(x_enc.device).unsqueeze(0)
            #enc_out, attns = self.encoder_list[i](enc_out, t, attn_mask=self.attn_mask_list[i].to(x_enc.device), channel_embed=channel_embed)
            enc_out, attns = self.encoder_list[i](enc_out, t, attn_mask=attn_mask, channel_embed=channel_embed)
