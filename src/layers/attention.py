...
class AttentionLayer_w_dispatchers(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        d_queries = self.query_projection_1(d).view(B, self.dispatchers, H, -1)
        keys = self.key_projection_1(keys).view(B, S, H, -1)
        values = self.value_projection_1(values).view(B, S, H, -1)
        ...
        d_out = d_out.view(B, self.dispatchers, -1)
        queries = self.query_projection_2(queries).view(B, L, H, -1)
        d_keys = self.key_projection_2(d_out).view(B, self.dispatchers, H, -1)
        d_values = self.value_projection_2(d_out).view(B, self.dispatchers, H, -1)
