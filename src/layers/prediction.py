...
class AttentivePredictionHead(nn.Module):
    ...
    def forward(self, x):
        ...
        queries = self.query_projection(pool_query).view(B * C, 1, H, self.d_keys)  # (batch * channel, 1, heads, d_keys)
        keys = self.key_projection(x).view(B * C, L, H, self.d_keys)  # (batch * channel, num_patches, heads, d_keys)
        values = self.value_projection(x).view(B * C, L, H, self.d_values)  # (batch * channel, num_patches, heads, d_values)
        ...
        pooled_values = pooled_values.reshape(B * C, -1).view(B, C, D)
