...
if __name__ == '__main__':
    ...
    parser.add_argument('--sample_multiplier', type=int, default=8,
                        help='sample multiple random timesteps for a batch, sample_multiplier * batch_size = total '
                             'samples')
    # parser.add_argument('--start_date', type=str, default='2000-01-01', help='start date of the dataset')
    # parser.add_argument('--end_date', type=str, default='2006-01-01', help='end date of the dataset (inclusive)')
    ...
    dataset_len = 10 * 252
    dataset_lapse = args.seq_len
    start_idx = np.arange(0, len(dates), dataset_lapse)
    end_idx = np.arange(dataset_len, len(dates), dataset_lapse)
    start_dates = [dates[i] for i in start_idx]
    end_dates = [dates[i] for i in end_idx]
    start_idx = start_idx[:len(end_idx)]

    for i in trange(len(start_idx)):
        args.start_date = start_dates[i]
        args.end_date = end_dates[i]

        exp = Exp(args)
        exp.train()
        exp.test(size=1024, method='discrete', temperature=0.7473231454237225, save_data=True,
                 sample_step=[0.45], joint_kl=False, lags=args.seq_len)